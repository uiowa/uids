...
content simply overflows it — and as of 2026-09-03 the headline size tracks height
(large→large, medium→medium, none→small ramp; the pre-title follows), so a small banner
no longer carries the display heading. The image is a labelled placeholder here; supply
a real one in production and tell the user it is a placeholder.
